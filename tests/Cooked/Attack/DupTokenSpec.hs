    ...
    [ testGroup "unit tests on a 'TxSkel'" $
        let attacker = wallet 6
            ...
            skelIn =
              txSkelTemplate
                { txSkelMints =
                    txSkelMintsFromList
                      [ (pol1, emptyRedeemer, tName1, 5),
                        (pol2, emptyRedeemer, tName2, 7)
                      ],
                  txSkelOuts =
                      ...
                      ( increment,
                        txSkelTemplate
                          { txSkelLabel = Set.singleton $ TxLabel DupTokenLbl,
                            txSkelMints =
                              txSkelMintsFromList
                                [ (pol1, emptyRedeemer, tName1, v1),
                                  (pol2, emptyRedeemer, tName2, v2)
                                ],
                            txSkelOuts =
                              [ paysPK (wallet 1) (Script.assetClassValue ac1 1 <> Script.lovelaceValueOf 1234),
                                paysPK (wallet 2) (Script.assetClassValue ac2 2),
      ...
      testCase "pre-existing tokens are left alone" $
        let attacker = wallet 6
            ...
            skelIn =
              txSkelTemplate
                { txSkelMints = txSkelMintsFromList [(pol, emptyRedeemer, tName1, 1)],
