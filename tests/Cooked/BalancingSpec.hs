...
spendsScriptUtxo True = do
  ...
  return $ Map.singleton scriptOutRef emptyRedeemer
  ...
  let skel =
        txSkelTemplate
          { txSkelOuts = List.filter ((/= mempty) . (^. txSkelOutValueL)) [paysPK bob toBobValue, paysPK alice toAliceValue],
            txSkelIns = additionalSpend <> Map.fromList ((,emptyRedeemer) <$> toSpendUtxos),
            txSkelOpts =
              optionsMod
                def
                  { txOptBalancingUtxos =
    ...
    validateTxSkel $
      txSkelTemplate
        { txSkelOuts = [paysPK bob (Script.lovelace (30_000_000 - maxFee))],
          txSkelIns = Map.singleton txOutRef emptyRedeemer,
          txSkelOpts =
            def
