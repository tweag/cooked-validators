...
instance Scripts.ValidatorTypes BigBoss where
  type RedeemerType BigBoss = RedeemerBigBoss
  type DatumType BigBoss = DatumBigBoss

bigBossTypedValidator :: Params -> Scripts.TypedValidator BigBoss
bigBossTypedValidator =
  Scripts.mkTypedValidatorParam @BigBoss
    $$(PlutusTx.compile [||validateBigBoss||])
    $$(PlutusTx.compile [||wrap||])
  where
    wrap = Scripts.wrapValidator @DatumBigBoss @RedeemerBigBoss

data Smith

instance Scripts.ValidatorTypes Smith where
  type RedeemerType Smith = RedeemerSmith
  type DatumType Smith = DatumSmith

smithTypedValidator :: Params -> Scripts.TypedValidator Smith
smithTypedValidator =
  Scripts.mkTypedValidatorParam @Smith
    $$(PlutusTx.compile [||validateSmith||])
    $$(PlutusTx.compile [||wrap||])
  where
    wrap = Scripts.wrapValidator @DatumSmith @RedeemerSmith
