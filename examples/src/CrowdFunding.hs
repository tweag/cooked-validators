...
validateSingleProposal name ctx =
  let txInfo = Ledger.scriptContextTxInfo ctx
      txInInfos = Ledger.txInfoInputs txInfo
      datums = findAllDatums txInfo
  ...
  where
    ...
    validateDatumsContent (Just _) _ (ProjectProposal {} : _) = False
...
validateRefunding pkh ctx =
  let txInfo = Ledger.scriptContextTxInfo ctx
   in Ada.getLovelace (Ada.fromValue $ flip Ledger.valuePaidTo pkh txInfo)
