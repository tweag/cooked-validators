...
-- 'TxSkelRedeemer'. The return value will be @Just@ the added data,
-- if anything changed.
...
-- | Add an input to a transaction. If the given 'Api.TxOutRef' is
-- already being consumed by the transaction, fail.
...
-- | Remove transaction inputs according to a given predicate. The
-- returned list contains all removed inputs.
...
-- | Ensure that a certain output is produced by a transaction. The
-- return value will be @Just@ the added output, when applicable.
...
-- | Add a transaction output, at the end of the current list of
-- outputs, thus retaining the initial outputs order.
addOutputTweak :: (MonadTweak m) => TxSkelOut -> m ()
addOutputTweak txSkelOut = overTweak txSkelOutsL (++ [txSkelOut])

-- | Remove transaction outputs according to some predicate. The
-- returned list contains all the removed outputs.
...
-- | Add a new entry to the 'TxSkelMints' of the transaction skeleton
-- under modification. As this is implemented in terms of
-- 'addToTxSkelMints', the same caveats apply as do to that function!
addMintTweak :: (MonadTweak m) => (Script.Versioned Script.MintingPolicy, MintsRedeemer, Api.TokenName, Integer) -> m ()
addMintTweak mint = overTweak txSkelMintsL $ addToTxSkelMints mint

-- | Remove some entries from the 'TxSkelMints' of a transaction,
-- according to some predicate. The returned list holds the removed
-- entries.
removeMintTweak ::
  (MonadTweak m) =>
  ((Script.Versioned Script.MintingPolicy, MintsRedeemer, Api.TokenName, Integer) -> Bool) ->
  m [(Script.Versioned Script.MintingPolicy, MintsRedeemer, Api.TokenName, Integer)]
