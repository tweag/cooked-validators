...
-- | Ensure that the transaction outputs have the necessary minimum
-- amount of Ada on them. This will only be applied if the
-- 'txOptEnsureMinAda' is set to @True@.
...
ensureTxSkelOutsMinAda skel = do
  ...
  where
    ...
    ensureTxSkelOutHasMinAda theParams txSkelOut@(Pays output) = do
      ...
      -- The following iterative approach to calculate the minimum Ada
      -- amount of a TxOut is necessary, because the additional value
      -- might make the TxOut heavier.
      --
      -- It is inspired by
...
--
-- holds. The fee depends on the transaction size, which might change
-- during the process of balancing, because additional inputs
-- belonging to the 'balancePK' might be added to ensure that
-- transaction inputs can cover all of the outputs. This means that
-- fee calculation and balancing are tied together. We follow
-- /plutus-apps/ in breaking this mutual dependency with a fixpoint
-- iteration, which should compute realistic fees.
--
--  This function also adjusts the transaction outputs to contain at
--  least the minimum Ada amount, if the 'txOptEnsureMinAda is @True@.
...
setFeeAndBalance balanceWallet skel0 collateralIns = do
  ...
  -- We start with a high startingFee, but theres a chance that 'w'
  -- doesn't have enough funds so we'll see an unbalanceable error; in
  -- that case, we switch to the minimum fee and try again.  That
  -- feels very much like a hack, and it is. Maybe we should witch to
  -- starting with a small fee and then increasing, but that might
  -- require more iterations until its settled.  For now, let's keep
  -- it just like the folks from plutus-apps did it.
  calcFee 5 (Fee 3_000_000) skel collateralUtxosPl
    `catchError` \case
      -- Impossible to balance the transaction
      MCEUnbalanceable _ _ ->
        -- WARN "Api.minFee" takes an actual Tx but we no longer
        -- provide it since we work on "TxSkel". However, for now, the
        -- implementation of "Api.minFee" is a constant of 10
        -- lovelace.
...
getCollateralInputs w = do
  ...
  case souts of
    ...
    -- TODO We only keep one element of the list because we are
    -- limited on how many collateral inputs a transaction can
    -- have. Should this be investigated further for a better
    -- approach?
...
data BalanceTxRes = BalanceTxRes
  { -- | Inputs that need to be added in order to cover the value in
    -- the transaction outputs
    newInputs :: [(Api.TxOutRef, Api.TxOut)],
    -- | The 'newInputs' will add _at least_ the missing value to
    -- cover the outputs, this is the difference of the input value
    -- together with the 'newInputs' and the output value.  This value
    -- must be nonnegative in every asset class.
    returnValue :: Api.Value,
    -- | Some additional UTxOs that could be used as extra
    -- inputs. These all belong to the same wallet that was passed to
    -- 'calcBalanceTx' as an argument, and are sorted in decreasing
    -- order of their Ada value.
    availableUtxos :: [(Api.TxOutRef, Api.TxOut)]
  }
  deriving (Show)

-- | Calculate the changes needed to balance a transaction with assets
-- from a given wallet. Every transaction that is sent to the chain
-- must be balanced: @inputs + mints == outputs + fee + burns@.
...
calcBalanceTx balanceWallet skel fee = do
  ...
  let outValue = txSkelOutputValue skel fee -- transaction outputs + fee + burned value
      ...
      -- This is the value that must still be paid by 'balancePK' in
      ...
      -- All TxOutRefs that the transaction consumes. We'll need them
      -- to make sure that no additional UTxOs are chosen that are in
      -- fact already present on the transaction.
      inputOrefs = Map.keys $ txSkelIns skel
  -- Get all UTxOs that belong to the given wallet, and that are not
  -- yet being consumed on the transaction.
  --
  -- These UTxOs are sorted in decreasing order of their Ada value,
  -- which will make 'selectNewInputs' will more likely select
  -- additional inputs that contain a lot of Ada. The hope behind this
  -- heuristic is that it'll therefore become less likely for the
  -- 'returnValue' to be less than the minimum Ada amount required for
  -- each output. See this comment for context:
  ...
  where
    ...
    selectNewInputs available chosen excess missing =
      case view flattenValueI missing of
        ...
        (ac, _) : _ ->
          -- Find the first UTxO belonging to the wallet that contains
          -- at least one token of the required asset class (The hope
          -- is that it'll contain at least @n@ such tokens, but we
          -- can't yet fail if there are fewer; we might need to add
          -- several UTxOs):
          case break ((`Script.geq` Script.assetClassValue ac 1) . outputValue . snd) available of
            ...
            (previousUtxos, theChosenUtxo : nextUtxos) ->
              let available' = previousUtxos ++ nextUtxos
               ...
               in -- A remark on why the following line should not
                  -- lead to an infinite recursion: The value
                  -- described by @missing'@ is strictly smaller than
                  -- the value described by @missing@, because there
                  -- was at least one token of the asset class @ac@ in
                  -- @theChosenValue@.
                  selectNewInputs available' chosen' excess' missing'

-- | Once we calculated what is needed to balance a transaction
-- @skel@, we still need to apply those changes to @skel@. Because of
-- the 'Script.minAdaTxOut' constraint, this might not be possible:
-- imagine the leftover is less than 'Script.minAdaTxOut', but the
-- transaction has no output addressed to the balancing wallet. If we
-- just create a new ouput for the balancing wallet and place the
-- leftover there, the resulting transaction will fail to validate
-- with "LessThanMinAdaPerUTxO" error. Instead, we need to consume yet
-- another UTxO belonging to the wallet to then create the output with
-- the proper leftover. If the wallet has no UTxO, then there's no way
-- to balance this transaction.
...
applyBalanceTx balancePK (BalanceTxRes newInputs returnValue availableUtxos) skel@TxSkel {..} = do
  -- Here we'll try a few things, in order, until one of them
  -- succeeds:
  --
  -- 1. If allowed by the balanceOutputPolicy, pick out the best
  --    possible output to adjust and adjust it as long as it remains
  --    with more than 'Script.minAdaTxOut'. No need for additional
  --    inputs apart from the @newInputs@. The "best possible" here
  --    means the most valuable ada-only output without any datum that
  --    will be paid to the given wallet. If the policy doesn't allow
  --    modifying an existing utxo or no such utxo exists, we move on
  --    to the next option;
  --
  -- 2. If the leftover is more than 'Script.minAdaTxOut' and (1)
  --    wasn't possible, create a new output to return leftover. No
  --    need for additional inputs besides the @newInputs@.
  --
  -- 3. Attempt to consume other possible utxos from 'w' in order to
  --    combine them and return the leftover.

  -- TODO: Mustn't every UTxO belonging to the wallet contain at least
  -- minAda?  In that case, we could forget about adding several
  -- additional inputs. If one isn't enough, there's nothing we can
  -- do, no?
  let bestOuts ::
