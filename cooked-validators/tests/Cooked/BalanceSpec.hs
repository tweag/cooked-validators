...
spec = do
  describe "spendValueFrom" $ do
    ...
    it "spends money from the output" $
      let txOut1 = outsOf 1 utxoIndex0 in
      shouldBe
    ...
    it "spends money from the outputs" $
      let Right (st,_) = tracePayWallet11 in
      let txOut11 = outsOf 11 $ mcstIndex st in
      shouldBe
...
wPKH i
  ...
  | i > 10  =
      let pk = Pl.generateFromSeed (fromString $ show i ++ pad) in
      walletPKHash (Pl.Wallet (Pl.MockWallet pk), pk)
  where
