...
instance ToCredential Pl.Address where
  toCredential = Pl.addressCredential
...
-- instance Pl.ToData a => ToOutputDatum a where
--   toOutputDatum = Pl.OutputDatum . Pl.Datum . Pl.toBuiltinData
...
class IsSpendableOut o where
  ...
  sOutStakingCredentialL :: Lens' o (Maybe Pl.StakingCredential)
  sOutDatumL :: Lens' o (DatumType o)
  sOutValueL :: Lens' o Pl.Value

sOutAddress :: (IsSpendableOut o, ToCredential (OwnerType o)) => o -> Pl.Address
sOutAddress o = Pl.Address (toCredential $ o ^. sOutOwnerL) (o ^. sOutStakingCredentialL)

sOutDatum :: (IsSpendableOut o, ToOutputDatum (DatumType o)) => o -> Pl.OutputDatum
sOutDatum = toOutputDatum . (^. sOutDatumL)
...
instance IsSpendableOut GenericSpendableOut where
  ...
  sOutStakingCredentialL =
    _2
      % lens
        Pl.addressStakingCredential
        (\addr new -> addr {Pl.addressStakingCredential = new})
  sOutDatumL = _3
  sOutValueL = _4
...
instance IsSpendableOut (PKSpendableOutWithDatum a) where
...
instance IsSpendableOut PKSpendableOutWithDatumHash where
...
instance IsSpendableOut (ScriptSpendableOutWithDatum a) where
...
instance IsSpendableOut (ScriptSpendableOutWithDatumHash a) where
