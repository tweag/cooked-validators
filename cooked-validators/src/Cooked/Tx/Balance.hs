...
spendValueFrom val utxos =
  ...
  foldl (\(spentTx, leftO) (curr,tok,i) -> addInputToBalance curr tok i spentTx leftO)
    ...
    (Pl.flattenValue val)
  where
    addInputToBalance :: Pl.CurrencySymbol -> Pl.TokenName -> Integer
                      -> [Pl.TxOutRef] -> Pl.Value -> ([Pl.TxOutRef], Pl.Value)
    addInputToBalance curr token i usedUTxO leftOver =
      ...
      then (usedUTxO, leftOver)
      else
        let thisTokLeftOver = Pl.valueOf leftOver curr token in
        ...
        then
          ...
          let prevLeftOver = leftOver Pl.- Pl.singleton curr token thisTokLeftOver in
          (usedUTxO ++ newUsedUTxOs, prevLeftOver <> additionalLeftover)
        -- If the leftover contains more (or exactly) money than required,
        -- then it is simply taken from the left-over.
        else (usedUTxO, leftOver Pl.- Pl.singleton curr token i)
...
-- >    == ([o1, o2, o3], 10)
--
necessaryUtxosFor :: Pl.CurrencySymbol -> Pl.TokenName -> Integer
                  -> [(Pl.TxOutRef, Pl.TxOut)] -> [Pl.TxOutRef] -> ([Pl.TxOutRef], Pl.Value)
...
necessaryUtxosFor curr token n ((oref, o):os) usedUTxO
  ...
  | otherwise =
      let foundAmount = valOf (Pl.txOutValue o) in
      let valLeftover = Pl.txOutValue o Pl.- Pl.singleton curr token foundAmount in
      ((oref :) *** (valLeftover Pl.+ )) $
        necessaryUtxosFor curr token (n - foundAmount) os usedUTxO

  where valOf val = Pl.valueOf val curr token
