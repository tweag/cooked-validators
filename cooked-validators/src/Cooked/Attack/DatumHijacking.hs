...
redirectScriptOutputTweak ::
  Is k A_Traversal =>
  Optic' k is TxSkel (L.TypedValidator a, Maybe L.StakingCredential, Either L.DatumHash (L.DatumType a), L.Value) ->
  -- | Return @Just@ the new validator, or @Nothing@ if you want to leave this
  -- output unchanged.
  (L.TypedValidator a -> Maybe L.StakingCredential -> Either L.DatumHash (L.DatumType a) -> L.Value -> Maybe (L.TypedValidator a)) ->
  ...
  -- with zero.
  (Integer -> Bool) ->
  Tweak [(L.TypedValidator a, Maybe L.StakingCredential, Either L.DatumHash (L.DatumType a), L.Value)]
...
datumHijackingAttack ::
  ...
  (L.TypedValidator a -> Either L.DatumHash (L.DatumType a) -> L.Value -> Bool) ->
