...
quantifyOverTraces ::
  ...
  ([Property] -> Property) ->
  Property
quantifyOverTraces i0 gParm showParm gTr prop collect =
  ...
  where
    ...
    go parm smc =
      let traces = interpretAndRunWith (runMockChainTFrom i0) smc
...
xxx ::
  ...
  (a -> StagedMockChain b) ->
  (a -> StagedMockChain b) ->
  (Either MockChainError (b, UtxoState) -> Either MockChainError (b, UtxoState)
-}
...
forAllTrPFrom ::
  ...
  (parm -> Either MockChainError (a, UtxoState) -> Property) ->
  Property
forAllTrPFrom i0 gParm gTr prop =
  quantifyOverTraces
    i0
    gParm
    show
...
-- | Specialized version of 'forAllTrPFrom' which does not generate a parameter
forAllTrFrom ::
  InitialDistribution ->
  GenTrace a ->
  (Either MockChainError (a, UtxoState) -> Property) ->
  Property
forAllTrFrom i0 gTr prop = forAllTrPFrom i0 (return ()) (const gTr) (const prop)

-- |Like 'forAllTrFrom', but runs from the default 'InitialDistribution'
forAllTr ::
  GenTrace a ->
  (Either MockChainError (a, UtxoState) -> Property) ->
  Property
forAllTr = forAllTrFrom def
