...
scIncrease sc =
  let auto = autoIncrease sc in
  sc { currentSlot = (if auto then (+1) else id) $ currentSlot sc }
...
scWait mSec sc =
  let slotLength = Pl.scSlotLength $ slotConfig sc in
  scWaitSlots (Pl.getPOSIXTime mSec `div` slotLength) sc

scSlotIs :: Integer -> SlotCounter -> SlotCounter
scSlotIs n sc =
  if n < currentSlot sc
  -- Time travel is a dangerous feature (Back to the Future should convince you about it),
  -- hence we do not allow it in the current version.
  then error "One cannot set time to a previous instant."
  else sc {autoIncrease = False, currentSlot = n}

scTimeIs :: Pl.POSIXTime -> SlotCounter -> SlotCounter
scTimeIs t sc =
  let s = Pl.posixTimeToEnclosingSlot (slotConfig sc) t in
  scSlotIs (Pl.getSlot s) sc
